...
This script is written for older NetworkManager builds that does not have support for ipv4.routing-rules, only ipv4.route-table
...
Linux route-metrics and only one gateway will allow traffic over one gateway. If Ethernet 1 is main gateway, connections will fail to Ethernet 2, sending intended traffic over Ethernet 1 gateway, not Ethernet 2's gateway.   
